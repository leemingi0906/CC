...
class RegTrainer(Trainer):
    def setup(self):
        ...
        self.datasets = {x: Crowd(os.path.join(args.data_dir, x),
                                  ...
                                  args.is_gray, x,
                                  alpha=getattr(args, 'alpha', 0.0)) for x in ['train', 'val']}
        ...
        for step, (inputs, points, targets, st_sizes) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                ...
                for feature in features:
                    ...
                    mean_sum = torch.sum(mean_feature**2, dim=2, keepdim=True)**0.5
                    cosine = 1 - torch.sum(feature*mean_feature, dim=2, keepdim=True) / (mean_sum * torch.sum(feature**2, dim=2, keepdim=True)**0.5 + 1e-5)
