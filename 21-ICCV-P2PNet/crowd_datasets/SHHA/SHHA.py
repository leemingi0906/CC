...
class SHHA(Dataset):
    def __init__(self, data_root, transform=None, train=False, patch=False, flip=False, use_npoint=False):
        ...
        for _, train_list in enumerate(self.img_list_file):
            ...
            with open(list_path) as fin:
                for line in fin:
                    ...
                    # [핵심 수정] 리스트 파일에 적힌 기존 절대 경로(/home/mingi/...)를 무시하고 
                    # 파일명(basename)만 추출하여 현재의 root_path와 결합합니다.
                    ...
                    # 실제 경로 재구성: data_root/part_A_final/.../images/파일명
        ...
        if self.nSamples == 0:
            print(f"❌ 오류: '{self.root_path}' 내에서 이미지를 찾지 못했습니다. 경로 구조를 확인하세요.")
    ...
    def __getitem__(self, index):
        ...
        if self.train and self.use_npoint:
            w, h = img.size 
            point = apply_npoint(point, (h, w), alpha=0.2, k=4)

        if self.transform is not None:
            img = self.transform(img)

        # P2PNet 데이터 증강 로직 (Random Scale/Crop/Flip)
        if self.train:
            scale_range = [0.7, 1.3]
